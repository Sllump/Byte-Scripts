...
Citizen.CreateThread(function()
  dispatchersOnDuty = RPC.execute("cool-dispatch:getDispatchersOnDuty")
end)

RegisterNetEvent("cool-dispatch:dispatchersOnDuty")
AddEventHandler("cool-dispatch:dispatchersOnDuty", function(pOnDuty)
	dispatchersOnDuty = 0 -- pOnDuty
end)

RegisterNetEvent("cool-dispatch:becomeDispatchSelf")
AddEventHandler("cool-dispatch:becomeDispatchSelf", function(pOnDuty)
    ...
    if pNotificationData.recipientList then
        for key, value in pairs(pNotificationData.recipientList) do
            if key == currentJob and value and not disableNotifications then
                ...
                elseif currentJob == "news" and exports["arp-inventory"]:getQuantity("scanner") > 0 then
                    sendNewsBlip(pNotificationData)
                end
                ...
                then
                    TriggerServerEvent('arp-fx:sound:playDispatch', pNotificationData.soundName);
                end
            end
        end
    end
...
function displayFastDispatch()
    if (currentJob == "police" or currentJob == "ems") and not isDead then
        ...
        TriggerEvent("arp-vehicles:isDispatchOpen", showDispatchLog)
        exports["cool-ui"]:SendUIMessage({
            source = "arp-nui",
            app = "dispatch",
            show = showDispatchLog,
            data = data or {},
        })
        exports["cool-ui"]:SetUIFocusCustom(showDispatchLog, showDispatchLog)
    end
end
...
Citizen.CreateThread(function()
    exports["arp-binds"]:registerKeyMapping("","Gov", "View Dispatch", "+showFastDispatch", "-showFastDispatch")
end)
...
local mapOpen = false
AddEventHandler("cool-ui:application-closed", function(name)
    if name == "dispatch" and showDispatchLog then
        print("is dispatch")
        showDispatchLog = false
        TriggerEvent("arp-vehicles:isDispatchOpen", true)
        mapOpen = false
        exports["cool-ui"]:SetInterfaceFocusCustom(false, false)
        print("set map open")
        TriggerServerEvent("cool-dispatch:mapIsOpen", mapOpen) -- most likely starts the map listener?
    end
end)

RegisterNetEvent("cool-dispatch:openFull")
AddEventHandler("cool-dispatch:openFull", function(pData)
    mapOpen = true
    exports["cool-ui"]:openApplication("dispatch", {
        showWithMap = true
    })
    TriggerServerEvent("cool-dispatch:mapIsOpen", mapOpen) -- most likely starts the map listener?
end)

RegisterNetEvent("cool-dispatch:updateUnits")
AddEventHandler("cool-dispatch:updateUnits", function(units)
    exports["cool-ui"]:sendAppEvent("dispatch", {
        action = "updateUnits",
        data = units,
    })
...
AddEventHandler('dispatch:clNotify', function(pNotificationData)
    ...
    if currentJob == "news" and exports["arp-inventory"]:getQuantity("scanner") > 0 then
...
RegisterNetEvent("cool-dispatch:updateUnitCoords")
AddEventHandler("cool-dispatch:updateUnitCoords", function(pCoords)
...
RegisterNetEvent("cool-dispatch:updateDispatch")
AddEventHandler("cool-dispatch:updateDispatch", function(pData)
  exports["cool-ui"]:sendAppEvent("dispatch", pData)
end)

RegisterUICallback("cool-ui:dispatchAction", function(data, cb)
    if data.action == "createCall" then
        RPC.execute("cool-dispatch:createCall", data.ctxId)
    end
    if data.action == "dismissPing" then
        RPC.execute("cool-dispatch:dismissPing", data.ctxId)
    end
    if data.action == "dismissCall" then
        RPC.execute("cool-dispatch:dismissCall", data.ctxId)
    end
    if data.action == "toggleUnit" then
        RPC.execute("cool-dispatch:toggleUnitAssignment", data.ctxId, data.unit)
    end
    if data.action == "setGPSLocation" then
        SetNewWaypoint(data.ping.origin.x, data.ping.origin.y)
    end
    if data.action == "setUnitVehicle" then
        RPC.execute("cool-dispatch:setUnitVehicle", data.data)
    end
    if data.action == "setUnitRidingWith" then
        RPC.execute("cool-dispatch:setUnitRidingWith", data.data)
    end
    cb({ data = {}, meta = { ok = true, message = "done" } })
end)

RegisterUICallback("cool-ui:getDispatchData", function(data, cb)
    local result = RPC.execute("cool-dispatch:getDispatchData")
    cb({ data = result, meta = { ok = true, message = result } })
end)

-- RegisterUICallback("cool-ui:dispatchToggleMapListener", function(data, cb)
--   mapOpen = false
--   TriggerServerEvent("cool-dispatch:mapIsOpen", mapOpen)
-- end)
      ...
      if (key ~= nil and item ~= nil) then
        ...
        if #(vector2(pos.x, pos.y) - vector2(item.pos.x, item.pos.y)) < 50.0 then
          if item.jobType == "ems" then
            if exports["arp-base"]:getModule("LocalPlayer"):getVar("job") == "ems" then
          ...
          elseif item.jobType == "news" then 
            if exports["arp-base"]:getModule("LocalPlayer"):getVar("job") == "news" then
              TriggerServerEvent('phone:assistRemove', item.id, item.jobType) -- Send message of clear to others
              clearBlip(item, false)
              if GetTimeDifference(curTime, item.timestamp) > 2000 then
                if canPay then
