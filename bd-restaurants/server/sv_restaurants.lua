...
RegisterNetEvent("bd-restaurants:triggerSilentAlarm")
AddEventHandler("bd-restaurants:triggerSilentAlarm", function(pCoords)
    local src = source
    -- bd-dispatch
end)

RPC.register("bd-restaurants:joinJob", function(pSource, pBiz, pType)
    local src = pSource
    local user = exports["bd-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local bizId = BUSINESSES[pBiz].id
    local isEmployed = exports["bd-business"]:IsEmployedAtBusiness(src, {business = {id = bizId} , character = char})
    if (isEmployed == false) then
        return false, "restaurant-no-available-positions", "There are no available positions."
    end

    if (#BUSINESSES[pBiz].employees >= MAX_EMPLOYEES) then
        return false, "restaurant-too-many-employees", "There are too many active employees."
    end

    BUSINESSES[pBiz].employees[src] = {name = char.first_name .. " " .. char.last_name, cid = char.id}
    return true, "restaurant-clocked-in", "Clocked in."
end)

RPC.register("bd-restaurants:leaveJob", function(pSource, pBiz)
    local src = pSource
    -- look thru all the businesses and remove the employee from the list
    ...
    return true, "restaurant-clocked-out", "Clocked out."
end)

RPC.register("bd-restaurants:getActiveEmployees", function(pSource, pBiz)
    return BUSINESSES[pBiz].employees
end)

RPC.register("bd-restaurants:setWorkHours", function(pSource, pBiz, pOpen, pClose)
...
RPC.register("bd-restaurants:getWorkHours", function(pSource, pBiz)
    local context = {
...
RPC.register("bd-restaurants:getSafeCash", function(pSource, pBiz)
    return getSafeCash(pBiz)
end)

RPC.register("bd-restaurants:takeSafeCash", function(pSource, pBiz)
...
RPC.register("bd-restaurants:depositCash", function(pSource, pBiz, pItemInfo)
...
RPC.register("bd-restaurants:crackSafe", function(pSource, pBiz)
    local safeCash = getSafeCash(pBiz)
    local query = "UPDATE _restaurants SET safeCash = 0 WHERE name = ? LIMIT 1"
    local queryData = Await(SQL.execute(query, pBiz))
...
RPC.register("bd-restaurants:startPurchase", function(pSource, pData)
    ...
    TriggerClientEvent("bd-restaurants:activePurchase", -1, ACTIVE_PURCHASES[pRegisterId])
end)

RPC.register("bd-restaurants:completePurchase", function(pSource, pData)
    local purchase = ACTIVE_PURCHASES[pData.registerId]
    purchase.biz = pData.biz
    if not purchase then return end
    local user = exports["bd-base"]:getModule("Player"):GetUser(pSource)
    ...
    if (pData.cash) then
        ...
        success = exports["bd-financials"]:DoCashTransaction(src, 1, "Services", price, "Bennys Motorworks", "purchase")
        if not success then
        ...
        -- bd-financials money remove from account
    ...
    TriggerClientEvent("bd-restaurants:closePurchase", -1, purchase.registerId)
    ACTIVE_PURCHASES[purchase.registerId] = nil

    return true
end)

RPC.register("bd-restaurants:getTakeoutBox", function(pSource, pData)
