...
local function getAppropriateFoodItems()
  ...
  local items = exports["bd-inventory"]:getFullItemList()
...
FoodItems = CacheableMap(function (ctx, pRestaurant)
  local items = RPC.execute("bd-restaurants:getFoodItems", pRestaurant)
...
AvailableMainDishes = CacheableMap(function (ctx, pRestaurant)
  local items = RPC.execute("bd-restaurants:fetchMenu", pRestaurant)
  if not items then return false, nil end
...
AddEventHandler("bd-restaurants:createFoodItem", function(p1, p2, pContext)
  local data = pContext.zones["restaurant_create_food_item"]
  ...
  for _, item in pairs(foodItems) do
    ...
    contextMenu[#contextMenu + 1] = {
      ...
      icon = "utensils",
      key = item,
      action = "bd-ui:restaurant:cookFoodItem",
...
RegisterUICallback("bd-ui:restaurant:cookFoodItem", function(data, cb)
  ...
  while true do
    if math.random() < 0.05 then
      local finished = exports["bd-ui"]:taskBarSkill(math.random(2000, 8000), math.random(7, 12))
    ...
    for _, item in pairs(items) do
      for _, ingred in pairs(uniqueIngredients) do
        if hasCategory(item.foodCategory, ingred) then
          ...
          local qty = exports["bd-inventory"]:getQuantity(item.itemName, true)
    ...
    local success = exports["bd-taskbar"]:taskBar(5000, "Preparing...", true, false, false, false, nil, 0.5, PlayerPedId())
    if success ~= 100 then break end
    local metaDatas = {}
    for ingred, items in pairs(currentIngredientCounts) do
      local processedCount = 0
      for itemName, count in pairs(items) do
        local itemData = exports["bd-inventory"]:getItemsOfType(itemName, count, true)
...
RegisterNetEvent("bd-restaurants:changedFoodItems", function(pRestaurant)
  resetFoodItems(pRestaurant)
end)

RegisterNetEvent("bd-restaurants:changedMenuItems", function(pRestaurant)
