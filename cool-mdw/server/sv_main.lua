...
RPC.register("loadProfile", function(pSource, pValue)
    ...
    local propertydata = Await(SQL.execute("SELECT * FROM properties WHERE cid = @cid",{ --AND propertycategory = @propertycategory
        ["cid"] = pValue.param,
...
--[[     local storagedata = Await(SQL.execute("SELECT * FROM business_storage WHERE storage_tenants = @storage_tenants",{
    ...
    for i = 1, #propertydata do
        properties[#properties + 1] = {
          ...
          ["property_name"] = propertydata[tonumber(i)].name,
          ["property_category"] = propertydata[tonumber(i)].cat
    }
    end
...
RPC.register("loadStaff", function(pSource, pValue)
    local staffdata = Await(SQL.execute("SELECT * FROM jobs_whitelist WHERE cid = @cid AND job = @job",{
        ["cid"] = pValue.param,
        ["job"] = "police"
    }))
    ...
    for i = 1, #staffdata do
        ...
        staff = {
            id = i,
            cid = staffdata[tonumber(i)].cid or 0,
            name = name,
            callsign = staffdata[tonumber(i)].callsign or 0,
            rank = tostring(staffdata[tonumber(i)].rank) or "1"
        }
    end
...
RPC.register("editStaff", function(pSource, pCid, pCallsign, pRank)
    local update = Await(SQL.execute("UPDATE jobs_whitelist SET callsign = @callsign, rank = @rank WHERE cid = @cid AND job = @job",{
        ["callsign"] = pCallsign.param,
        ["rank"] = pRank.param,
        ["id"] = pCid.param,
        ["job"] = "police"
    }))
...
RPC.register("fetchProperties", function(pSource)
    local data = Await(SQL.execute("SELECT * FROM properties ORDER BY id ASC",{}))
...
RPC.register("fetchOfficers", function(pSource)
    local officerdata = Await(SQL.execute("SELECT * FROM jobs_whitelist WHERE job = @job", {
        ["job"] = "police"
    }))
...
RPC.register("fetchMdwInfo", function(pSource, pJob)
    ...
    return user:getCurrentCharacter().first_name, user:getCurrentCharacter().last_name, tostring(data[1].rank), label, data[1].callsign
end)
...
RPC.register("hireStaff", function(pSource, pCid, pCallsign, pRank)
    local cid = tonumber(pCid.param)
    local rank = tonumber(pRank.param)
    local callsign = tostring(pCallsign.param)
    local job = "police"

    local insert = Await(SQL.execute("INSERT INTO jobs_whitelist (cid, rank, callsign, job) VALUES (@cid, @rank, @callsign, @job)", {
        ["cid"] = cid,
        ["rank"] = rank,
        ["callsign"] = callsign,
        ["job"] = job
...
RPC.register("fireStaff", function(pSource, pCid)
    local delete = Await(SQL.execute("DELETE FROM jobs_whitelist WHERE cid = @cid AND job = @job", {
        ["cid"] = pCid.param,
        ["job"] = "police"
    }))
...
RegisterCommand("addcharge", function(source, args, raw)
    local user = exports["arp-base"]:getModule("Player"):GetUser(source)
