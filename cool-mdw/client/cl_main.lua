...
local function playAnimation()
  ...
  AddEventHandler("arp-mdw:openMDW", function(data)
  ...
  RegisterNetEvent("arp-ui:restartUI")
  AddEventHandler("arp-ui:restartUI", function()
    StopAnimTask(PlayerPedId(), "amb@code_human_in_bus_passenger_idles@female@tablet@base", "base", 1.0)
    TriggerEvent("destroyPropPhone")
    SetPlayerControl(PlayerId(), 1, 0)
    SetNuiFocus(false, false)
  ...
  RegisterNUICallback('arp-mdw:searchProfiles', function(data, cb)
    local value = data.value
    local data = RPC.execute("searchProfiles", value)
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('arp-mdw:searchVehicles', function(data, cb)
    local value = data.value
    local data = RPC.execute("searchVehicles", value)
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:loadProfile', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('arp-mdw:loadVehicle', function(data, cb)
    local value = data.value
    local vehicle = RPC.execute("loadVehicle", value)
    cb({ data = {vehicle = vehicle} or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('arp-mdw:loadStaff', function(data, cb)
    local value = data.value
    local staff = RPC.execute("loadStaff", value)
    cb({ data = {staff = staff} or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:editProfile', function(data, cb)
  ...
  RegisterNUICallback('arp-mdw:editVehicle', function(data, cb)
    local success, message = RPC.execute("editVehicle", data.id, data.image, data.info)
    local vehicle = RPC.execute("loadVehicle", data.id)
    cb({ data = {vehicle = vehicle} or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('arp-mdw:editStaff', function(data, cb)
    local success, message = RPC.execute("editStaff", data.id, data.callsign, data.rank)
    local staf = RPC.execute("loadStaff", data.id)
    cb({ data = {staf = staf} or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:loadIncident', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('arp-mdw:loadIncidentEms', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('arp-mdw:loadEvidence', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('arp-mdw:updateIncident', function(data, cb)
    local data = RPC.execute("updateIncident", data.id, data.type, data.value, data.secondValue, data.add)
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('arp-mdw:updateIncidentEms', function(data, cb)
    local data = RPC.execute("updateIncidentEms", data.id, data.type, data.value, data.secondValue, data.add)
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('arp-mdw:saveCriminal', function(data, cb)
    local data = RPC.execute("saveCriminal", data.id, data.cid, data.value)
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:fetchBusinesses', function(data, cb)
    local data = RPC.execute("fetchBusinesses")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:loadBusiness', function(data, cb)
    local value = data.value
    local employees, count = RPC.execute("loadBusiness", value)
    cb({ data = {employees = employees, count = count} or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:fetchProperties', function(data, cb)
    local data = RPC.execute("fetchProperties")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:fetchCharges', function(data, cb)
    local data = RPC.execute("fetchCharges")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:fetchOfficers', function(data, cb)
    local data = RPC.execute("fetchOfficers")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('arp-mdw:fetchStaff', function(data, cb)
    local data = RPC.execute("fetchStaff")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('arp-mdw:fetchEms', function(data, cb)
    local data = RPC.execute("fetchEms")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:fetchIncidents', function(data, cb)
    local data = RPC.execute("fetchIncidents")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('arp-mdw:fetchIncidentsEms', function(data, cb)
    local data = RPC.execute("fetchIncidentsEms")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:fetchEvidence', function(data, cb)
    local data = RPC.execute("fetchEvidence")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('arp-mdw:fetchWarrants', function(data, cb)
    local data = RPC.execute("fetchWarrants")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('arp-mdw:saveIncident', function(data, cb)
  ...
  RegisterNUICallback('arp-mdw:saveIncidentEms', function(data, cb)
  ...
  RegisterNUICallback('arp-mdw:assignLicense', function(data, cb)
    local result = RPC.execute("assignLicense", data.cid, data.type)
    if not result then cb(false) return end
    cb(true)
  end)

  RegisterNUICallback('arp-mdw:removeLicense', function(data, cb)
    local result = RPC.execute("removeLicense", data.cid, data.type)
    if not result then cb(false) return end
    cb(true)
  end)

  RegisterNUICallback('arp-mdw:createWarrant', function(data, cb)
  ...
  RegisterNUICallback('arp-mdw:deleteWarrant', function(data, cb)
  ...
  RegisterNUICallback('arp-mdw:hireStaff', function(data, cb)
  ...
  RegisterNUICallback('arp-mdw:fireStaff', function(data, cb)
