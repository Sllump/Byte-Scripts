;(function () {
    !(function () {
      ...
      onNet('bd-bulletclub:gameStarted', (_0x5ad20c, _0x4fdc60, _0x5884b1) => {
          return _0x278f2e(_0x5ad20c, _0x4fdc60, _0x5884b1)
        }
      )
      
      onNet('bd-bulletclub:startGame', () => {
        const PlayerID = GetPlayerServerId(PlayerId())
        return emitNet('bd-bulletclub:startGame', PlayerID)
      })

      onNet('bd-bulletclub:modifySettings', () => {
        return modifySettings()
      })
      onNet('bd-bulletclub:viewLeaderBoard', (leaderboard) => {
        _0x4890c3(leaderboard.type)
      })

      onNet('bd-bulletclub:getWeapon', () => {
        getWeapon()
      })

      onNet('bd-bulletclub:resetGameState', () => {
        emitNet('bd-bulletclub:resetGameState')
      })

      onNet('bd-bulletclub:gameReset', () => {
        gameReset()
      })

      RegisterInterfaceCallback('bd-bulletclub:purchaseWeapon', (data, _0x3c4f20) => {
          const _0x4470e0 = { ok: true, message: 'done' },
            _0x7d11ee = { data: {}, meta: _0x4470e0 }
          _0x3c4f20(_0x7d11ee)
          const GetItemMeta = exports['bd-inventory'].GetItemsByItemMetaKV(data?.key?.weapon, 'BulletClub', true)
      ...
      onNet('bd-bulletclub:updateHud', (_0x21a871) => {
        if (!pZone('bullet_club')) {
          return
        }
        const hudStatus = { show: true, title: 'Bullet Club Range', values: _0x21a871, }
        exports['bd-interface'].sendAppEvent('status-hud', hudStatus)
      })
      ...
      const TargetPeds = [
        ...
        _0x278f2e = async (_0x3531be, _0x2e82bb, _0x34dd06) => {
          ...
          const Targets = _0x54a5e1(_0x3531be)
          ...
          const gameTimer = setInterval(() => {
            ...
            if (_0x2b4ee2 && IsEntityDead(_0x2b4ee2)) {
              if (GetPedSourceOfDeath(_0x2b4ee2) === PlayerPedId()) {
                targetsKilled += 1
                emitNet('bd-bulletclub:targetKilled')
              }
              DeleteEntity(_0x2b4ee2)
              _0x2b4ee2 = null
              const _0x14f44b = _0x17f261(Targets, _0x34dd06)
              Targets.splice(_0x14f44b, 1)
              emitNet('bd-bulletclub:updateEnemiesRemaining', Targets.length)
            }

            if (GetGameTimer() > _0x5971ad + _0x2e82bb && Targets.length > 0) {
              DeleteEntity(_0x2b4ee2)
              const _0x294bd7 = _0x17f261(Targets, _0x34dd06)
              Targets.splice(_0x294bd7, 1)
              emitNet('bd-bulletclub:updateEnemiesRemaining', Targets.length)
            }
            ...
            if (
              ...
              DeleteEntity(_0x2b4ee2)
              emit('DoLongHudText', 'Completed! Score: ' + targetsKilled, 1, 12000)
              const player = GetPlayerServerId(PlayerId())
              emitNet('bd-bulletclub:gameCompleted', player, targetsKilled)
              emitNet('bd-bulletclub:updateEnemiesRemaining', Targets.length)
              _0x5d255e()
        ...
        modifySettings = async () => {
          ...
          const easyMode = {
            ...
            selectDifficulty = {
              ...
              icon: 'time',
              _type: 'select',
              options: [easyMode, mediumMode, hardMode, aimbotMode],
            },
            True = {
              id: 'true',
              name: 'True',
            ...
            _0x1253f2 = [selectDifficulty, strafeEnabled],
            sGame = await exports['bd-interface'].OpenInputMenu(_0x1253f2)
          if (!sGame) {
            return
          }

          const sDifficulty =
              (_0x2147dc = sGame.difficulty) !== null && _0x2147dc !== void 0
                ? _0x2147dc
                : 'easy',
            sStrafe = sGame.strafeEnabled === 'true',
            sSettings = {
              difficulty: sDifficulty,
              strafeEnabled: sStrafe,
            }
          emitNet('bd-bulletclub:updateSettings', sSettings)
        },

        _0x4890c3 = async (_0x566cdf) => {
          const _0x24f0f5 = await _0x38b1ca.Procedures.execute('bd-bulletclub:getLeaderBoard', _0x566cdf),
          ...
          exports['bd-interface'].showContextMenu(_0x3cbfda)
        },
        ...
        getWeapon = () => {
          ...
          for (const _0x1ba8b2 of weaponList) {
            const _0xfe183a = {
              ...
              action: 'bd-bulletclub:purchaseWeapon',
              key: { weapon: _0x1ba8b2.item },
            }
            _0x18d817.push(_0xfe183a)
          }
          exports['bd-interface'].showContextMenu(_0x18d817)
        },
      ...
      onNet('bd-polyzone:enter', (bulletclubWeapon) => {
        Weapons.add(bulletclubWeapon)
      })

      onNet('bd-polyzone:exit', (bulletclubWeapon) => {
        Weapons.delete(bulletclubWeapon)
      })

      onNet('bd-polyzone:enter', (zone) => {
        if (zone !== 'bullet_club') {
          return
        }
        emitNet('bd-bulletclub:enteredBuilding')
        SetPedInfiniteAmmoClip(PlayerPedId(), true)
      })

      onNet('bd-polyzone:exit', (zone) => {
          if (zone !== 'bullet_club') {
            return
          }
        exports['bd-interface'].sendAppEvent('status-hud', _0x208bee)
      ...
      const _0x17f261 = (_0x218b36, _0x336906) => {
        ...
        initializeZones = () => {
          ...
          coordsMisc = { heading: 0, minZ: 18.43, maxZ: 22.83, zoneEvents: 'bd-bulletclub:updateHud', data: { id: 'bullet_club' }, debugPoly: false }
          exports['bd-polyzone'].AddBoxZone('bullet_club', coords, 22.9, 24.6, coordsMisc)
          ...
          exports['bd-polytarget'].AddBoxZone('bullet_club_controls', controlsCoords, 3.85, 0.3, controlsCoords2)
          ...
          exports['bd-polytarget'].AddBoxZone('bullet_club_leaderboards', leaderboardCoords, 1.5, 0.2, leaderboardCoords2)
        },

        initializePeekEntries = () => {
          console.log('[Bullet Club] Initializing peek entries...')
          const startGame = {
              triggerEvent: 'bd-bulletclub:startGame',
              id: 'bullet_club_start',
              icon: 'play',
              label: 'Start Game ($500)',
            },
            distance = { distance: { radius: 3.5 } }
          exports['bd-interact'].AddPeekEntryByPolyTarget('bullet_club_controls', [startGame], distance)

          const resetGame = {
            triggerEvent: 'bd-bulletclub:resetGameState',
            id: 'bullet_club_reset',
            icon: 'exclamation',
            label: 'Force Reset Game',
          }
          exports['bd-interact'].AddPeekEntryByPolyTarget('bullet_club_controls', [resetGame],
            {
              distance: { radius: 3.5 },
              isEnabled: async () => {
                const isEmployed = await exports['bd-business'].IsEmployedAt('the_bullet_club')
                return isEmployed
              },
            }
          )
          ...
          const changeSettings = {
            triggerEvent: 'bd-bulletclub:modifySettings',
            id: 'bullet_club_setting',
            icon: 'cog',
            label: 'Change Settings',
          }

          exports['bd-interact'].AddPeekEntryByPolyTarget('bullet_club_controls', [changeSettings],
            {
              distance: { radius: 3.5 },
              isEnabled: async () => {
                const isEmployed = await exports['bd-business'].IsEmployedAt('the_bullet_club')
          ...
          const getWeapon = {
            triggerEvent: 'bd-bulletclub:getWeapon',
            id: 'bullet_club_getweapon',
            icon: 'hand-paper',
            label: 'Get Weapon',
          }
          exports['bd-interact'].AddPeekEntryByPolyTarget( 'bullet_club_controls', [getWeapon], distance )
          
          const leaderboardEasy = {
              triggerEvent: 'bd-bulletclub:viewLeaderBoard',
          ...
          exports['bd-interact'].AddPeekEntryByPolyTarget('bullet_club_leaderboards',
