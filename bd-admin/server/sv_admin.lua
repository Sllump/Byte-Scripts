...
function getCommand(pName)
    for idx, command in ipairs(BD.Admin.Commands) do
...
function getCommandUI(pSource)
    ...
    for idx, command in ipairs(BD.Admin.Commands) do
        if (userPower >= command.rank) then
            command.commandUI.adminMenu.command.action = command.name
            table.insert(commands, command.commandUI)
        end
    end
...
function getCurrentPlayers()
    ...
    for idx, player in ipairs(players) do
        ...
        local user = exports["bd-base"]:getModule("Player"):GetUser(player)
...
function runCommandFromClient(pSource, pAction, pData)
    ...
    if command.isClientCommand then
        TriggerClientEvent("bd-admin:runClientCommand", pSource, pAction, pSource, pData, command.log)
    else
