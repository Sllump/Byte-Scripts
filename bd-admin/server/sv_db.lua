BD.Admin.DB = BD.Admin.DB or {}

function BD.Admin.DB.getAllIdentifiers(src)
    ...
    -- use the function called BD.Admin.Util.getIdentifier to get the identifiers
    -- return the identifiers in a table
    for k, v in pairs(targetIdentifiers) do
        local identifier = BD.Admin.Util.getIdentifier(src, v)
        if identifier then
...
function BD.Admin.DB.IsPlayerBanned(self, src)
    local pSteamId = BD.Admin.Util.getIdentifier(self, src, "steam")
    local pLicense = BD.Admin.Util.getIdentifier(self, src, "license")
    local pLicense2 = BD.Admin.Util.getIdentifier(self, src, "license2")
    local pXbox = BD.Admin.Util.getIdentifier(self, src, "xbl")
    local pLive = BD.Admin.Util.getIdentifier(self, src, "live")
    local pDiscord = BD.Admin.Util.getIdentifier(self, src, "discord")
    local pCFX = BD.Admin.Util.getIdentifier(self, src, "fivem")
    local pIp = BD.Admin.Util.getIdentifier(self, src, "ip")
    local pTokens = BD.Admin.Util.getUserToken(self, src)
...
function BD.Admin.DB.unbanUser(pBanId)
    if pBanId == nil then
        return
    end
...
function BD.Admin.DB.addBanHistory(pBanId)
    if pBanId == nil then
        Wait(1000)
    end
...
AddEventHandler("bd-base:exportsReady", function()
    exports["bd-base"]:addModule("Admin", BD.Admin)
end)
