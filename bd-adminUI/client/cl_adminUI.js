...
function toggleNUI(){
    ...
    SendNuiMessage(JSON.stringify({
        action: 'setKeyBinds',
        selectionMenu: global.exports["bd-keybinds"].getKeyMapping("+enableSelectionMenu"),
        selectionKey: global.exports["bd-keybinds"].getKeyMapping("+adminSelect")
    }))
}
function exitNUI()
{
...
on('__cfx_nui:closeMenu', (data, cb) => {
    ...
    if(data.toSelect){
        global.exports["bd-admin"].enterSelection();
    }
...
on('__cfx_nui:insideCommand', (data, cb) => {
    ...
    if(data.inside){
        ...
        emit('bd-binds:should-execute',false)
        ...
        emit('bd-binds:should-execute',true)
        if(blockingActions){
...
global.exports["bd-keybinds"].registerKeyMapping("", "zzAdmin", "Selection Menu", "+enableSelectionMenu", "-enableSelectionMenu", "")
