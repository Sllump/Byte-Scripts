window.onload = function(e)
{
    ...
    function toggleMenu(state, menu)
    {
        if(state)
        {
            if(menuStructure[menu].selectedItem < maxMenuItems)
            {
                ...
                $.post("http://bd-bennys/selectedItem", JSON.stringify({
                    id: val1,
                    item: val2,
                    item2: val3
                }));
    ...
    function updateItem2TextOnly(menu, id, text)
    {
        ...
        $.post("http://bd-bennys/updateItem2", JSON.stringify({
            item: text
        }));
    }
    ...
    function updateReceiptData(data)
    {
        const receiptItem = $(".receipt-items")
        receiptItem.empty();

        if (!data && !data.upgrades && data.length === 0) {
            toggleReceiptData(false)
            return;
        }

        data.upgrades = data.upgrades?.filter(x => x);
        if (data.upgrades === undefined || data.upgrades.length === 0) {
            toggleReceiptData(false)
        } else {
            toggleReceiptData(true)
        }

        data.upgrades?.forEach((item) => {
            if (!item) return null;
            receiptItem.append(`
                <div class='receipt-item'>${item.modData?.modName || "Unknown Item?"}</div>
            `)
        });

        const price = data.upgrades.map(x => x.price).reduce((x, y) => { return x + y }, 0)
        $(".receipt-cost").html("$" + price)

        if (receiptItem.prop("offsetHeight") < receiptItem.prop("scrollHeight") ) {
    ...
    function toggleReceiptData(on) {
        $(".receipt-container").css("display", on ? "block" : "none");
    }
    ...
    window.addEventListener("message", function(event)
    {
        ...
        if(eventData.updateReceipt)
        {
            updateReceiptData(eventData.receipt);
        }

        if (eventData.scrollReceiptDown) {
            const currentScroll = $(".receipt-items").scrollTop();
            $(".receipt-items").scrollTop(currentScroll + 2)
        }

        if (eventData.scrollReceiptUp) {
            const currentScroll = $(".receipt-items").scrollTop();
            $(".receipt-items").scrollTop(currentScroll - 2)
        }
    });
