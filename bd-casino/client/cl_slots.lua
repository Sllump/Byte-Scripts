...
--   local balance = RPC.execute("bd-casino:getCurrentChipCount")
--  --local balance = 400
--   local values = {}
    if state then 
...
--     -- exports["bd-interface"]:sendAppEvent("status-hud", {
--     --   show = state,
--     --   title = title,
--     --   values = values
--     -- })
--     exports["bd-textdisplay"]:DisplayText({
--       header = title,
--         rows = {
--           {
--               id = 0, 
...
local function getUpFromSlotMachine()
  ...
  TriggerServerEvent("bd-slotmachines-sv:getUpFromSlotMachine")
  drawStatusHUD(false, {})
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "bd-interface-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "bd-interface-slotmachines:leaveMachine",
            title = "Leave Machine",
            key = {},
          })

          exports["bd-interface"]:showContextMenu(data) 
        end
      end
    end
...
local function initializeSlotMachines()
  ...
  TriggerServerEvent("bd-slotmachines-sv:getMachineData")
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["bd-interface"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            if exports["bd-inventory"]:hasEnoughOfItem("casinomember", 1, false, true) then
            ...
            exports["bd-interface"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
        
          if IsControlJustReleased(1, 38) and canPlayAtSlotMachine then --Key: E
            TriggerServerEvent("bd-slotmachines-sv:sitDownAtSlotMachine", k)
...
--#[Event Handlers]#--
RegisterNetEvent("bd-slotmachines-cl:getMachineData")
AddEventHandler("bd-slotmachines-cl:getMachineData", function(data)
...
RegisterNetEvent("bd-slotmachines-cl:updateMachineData")
AddEventHandler("bd-slotmachines-cl:updateMachineData", function(machine, ply)
...
RegisterNetEvent("bd-slotmachines-cl:sitDownAtSlotMachine")
AddEventHandler("bd-slotmachines-cl:sitDownAtSlotMachine", function(machine, coinValue)
  local plyPed = PlayerPedId()
  local plyPos = GetEntityCoords(plyPed)
...
RegisterNetEvent("bd-slotmachines-cl:playSlotMachineSound")
AddEventHandler("bd-slotmachines-cl:playSlotMachineSound", function(machinePos, machineModel, sound)
  if isSMSetup then
    local soundRef = ""
...
RegisterNetEvent("bd-slotmachines-cl:stopReels")
AddEventHandler("bd-slotmachines-cl:stopReels", function(reel, rot)
...
RegisterNetEvent("bd-slotmachines-cl:spinFinished")
AddEventHandler("bd-slotmachines-cl:spinFinished", function(amountWon)
  ...
  if amountWon < 500 then
    ...
    -- RPC.execute("bd-casino:winChips", amountWon)
    TriggerEvent('bd-casino:giveChipsCL', amountWon)
    TriggerServerEvent('bd-casino:slotsLogWin', amountWon)
    pTimesWon = pTimesWon + 1

    TriggerEvent("DoLongHudText", "You've won $" .. amountWon .. "!", 1)
  else
    TriggerEvent('bd-casino:addBalance', '100')
...
RegisterNetEvent("bd-slotmachines-cl:toggleSlotMachines")
AddEventHandler("bd-slotmachines-cl:toggleSlotMachines", function(state)
...
AddEventHandler("bd-casino:casinoEnteredEvent", function()
  isInsideCasino = true
...
AddEventHandler("bd-casino:casinoExitedEvent", function()
...
--#[NUI Callbacks]#--
RegisterInterfaceCallback("bd-interface-slotmachines:placeBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local chipAmt = RPC.execute('bd-casino:getChips')
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    if chipAmt >= currentBet then 
      TriggerEvent('bd-casino:removeChipsCL', currentBet)
    else
    ...
    -- local info = (exports["bd-inventory"]:GetInfoForFirstItemOfName("casinoloyalty") or { information = "{}" })
    TriggerServerEvent(
      "bd-slotmachines-sv:spinMachine",
    ...
    --local balance = RPC.execute("bd-casino:getCurrentChipCount")
    local balance = RPC.execute('bd-casino:getChips')
    --local values = {}
    --table.insert(values, "Balance: $" .. balance)
...
RegisterInterfaceCallback("bd-interface-slotmachines:leaveMachine", function(data, cb)
...
function spinSlot(tickRate)
  ...
  for i=1,300,1 do
    ...
    elseif i == 180 then
      ...
      TriggerEvent("bd-slotmachines-cl:stopReels", 1, ang1)
    end
    if i < 240 then
    elseif i == 240 then
      ang2 = tickRate.b*22.5-180+0.0
      print(ang2)
      TriggerEvent("bd-slotmachines-cl:stopReels", 2, ang2)
    end
    if i < 300 then
    elseif i == 300 then
      ang3 = tickRate.c*22.5-180+0.0
      TriggerEvent("bd-slotmachines-cl:stopReels", 3, ang3)
    end				
    Citizen.Wait(10)
  end
  TriggerServerEvent('bd-slotmachines-sv:slotsCheckWin', tickRate, data)
end

RegisterNetEvent('bd-slotmachines-cl:startSpin')
AddEventHandler('bd-slotmachines-cl:startSpin',function(tickRate)
  spinSlot(tickRate)
end)
...
Citizen.CreateThread(function()
  exports["bd-polyzone"]:AddBoxZone("casino_door_entry", vector3(959.33, 33.32, 71.91), 2, 5, {
    heading=330,
    --debugPoly=true,
    minZ=69.91,
    maxZ=73.91
...
RegisterNetEvent('bd-polyzone:enter')
AddEventHandler('bd-polyzone:enter', function(name)
