                ...
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then
                        if exports['bd-inventory']:hasEnoughOfItem('casinomember', 1) then
                            exports["bd-interface"]:showInteraction("Press [E] to play blackjack", 'info')
                        else
                            exports["bd-interface"]:showInteraction("Press [E] to play blackjack", 'error')
                        end
                    else 
                        exports["bd-interface"]:showInteraction("This seat is taken")
                        Citizen.Wait(3000)
    ...
    while true do 
        ...
        if drawCurrentHand then 
            ...
            local pValues = {
                ...
                "Dealers hand is: " .. dealersHand,
            }
              
            exports["bd-interface"]:sendAppEvent("status-hud", {
                show = true,
                title = "Blackjack $" .. InputAmt ..".00",
                values = pValues 
            })
...
AddEventHandler("Blackjack:successBlackjackBet",function()
    ...
    TriggerEvent("bd-blackjack:close")
            ...
            if IsControlJustPressed(0, 38) then --Sit down [E]
                if blackjackTableData[closestChair] == false then
                    if exports['bd-inventory']:hasEnoughOfItem('casinomember', 1) then
                        ...
                        TriggerEvent('DoLongHudText', 'You need a Casino Membership to play Blackjack.', 2)
                    end
                end
            end
        end
...
AddEventHandler("Blackjack:sitAtBlackjackTable",function(chair)
    ...
    TriggerEvent('bd-hud:show_money')
end)
    ...
    while true do
        if sittingAtBlackjackTable and canExitBlackjack then
            ...
            if IsControlJustPressed(0, 202) and not waitingForSitDownState then  --Leave seat [backspace]
                TriggerEvent("bd-blackjack:close")
                TriggerEvent('bd-hud:hide_money')
...
Citizen.CreateThread(function()
    while true do
        if drawTimerBar then
            if timeLeft == 15 then 
                exports['bd-interface']:showInteraction('Time: '..timeLeft)
                Citizen.Wait(2500)
                exports['bd-interface']:hideInteraction()
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            TriggerEvent("bd-context:closemenu")
        end
    end)
end)
...
AddEventHandler("betmenu", function()
    exports['bd-interface']:openApplication('textbox', {
        callbackUrl = 'bd-blackjack:betamount',
        key = 1,
        items = {
          {
            icon = "money-bill",
...
RegisterInterfaceCallback('bd-blackjack:betamount', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local chips = RPC.execute('bd-casino:getChips')
    InputAmt = data.values.pBAmount
    print(chips)
    if tonumber(InputAmt) >= 0 and chips >= InputAmt then
        if tonumber(InputAmt) > 5000 then
            TriggerEvent('DoLongHudText', 'Max Betting is 5k', 2)
            return
        end
        if tonumber(InputAmt) < 25001 then
            TriggerServerEvent("Blackjack:setBlackjackBet",globalGameId,InputAmt,closestChair)
            Citizen.Wait(5000)
...
AddEventHandler('casinomenu', function()
	...
	local menuCasino = {
		{
            title = "Hit",
            description = "Big Ballz !",
            key = "BJ.Hit",
			action = 'bd-blackjack:hit',
        },
		{
            title = "Stand",
            description = "Pussio Fam !",
            key = "BJ.Stand",
			action = 'bd-blackjack:stand',
        },
    }
    exports["bd-interface"]:showContextMenu(menuCasino)
end)

RegisterInterfaceCallback('bd-blackjack:stand', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('standcasino')
end)

RegisterInterfaceCallback('bd-blackjack:hit', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('hitcasino')
end)
...
RegisterNetEvent("bd-blackjack:close", function()
    local pValues = {
        "line 1",
        "line 2",
    }
