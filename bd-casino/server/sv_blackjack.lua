    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
        ...
        if not blackjackGameInProgress[gameId] then
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    TriggerClientEvent('bd-casino:removeChipsCL', source, betAmount)
                                        ...
                                        if currentHand ~= nil then
                                            if currentHand <= 21 then
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    TriggerClientEvent('bd-casino:giveChipsCL', source, potentialWinAmount)
                                                ...
                                                elseif currentHand == dealerHand then                                                  
                                                    ...
                                                    PerformHttpRequest("YOUR-WEBHOOK-GOES-HERE", function(err, text, headers) end, 'POST', json.encode({username = "Byte Development | Casino", embeds = connect, avatar_url = "AVATAR-URL"}), { ['Content-Type'] = 'application/json' })
