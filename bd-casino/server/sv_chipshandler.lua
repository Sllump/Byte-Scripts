...
RegisterServerEvent('bd-casino:giveChips')
AddEventHandler('bd-casino:giveChips', function(amount)
    local src = source
    local user = exports['bd-base']:getModule("Player"):GetUser(src)
    local StateID = user:getCurrentCharacter().id
...
RegisterServerEvent('bd-casino:removeChips')
AddEventHandler('bd-casino:removeChips', function(pRemoveAmt)
    local src = source
    local user = exports['bd-base']:getModule("Player"):GetUser(src)
    local StateID = user:getCurrentCharacter().id
...
RPC.register("bd-casino:getChips", function(pSource)
    local src = source
    local user = exports["bd-base"]:getModule("Player"):GetUser(src)
