...
-- RegisterNUICallback('cool-hud:hudSetPreferences', function(data, cb)
...
--     TriggerEvent("cool-preferences:setPreferences", data)
-- end)

AddEventHandler('cool-preferences:setPreferences', function(pData)
    exports["arp-interface"]:sendAppEvent('preferences', pData)
end)
...
local function toggleCompass()
    ...
    print("compassEnabled: " .. tostring(compassEnabled))
    print("showCompassFromWatch: " .. tostring(showCompassFromWatch))
    print("showCompassFromCar: " .. tostring(showCompassFromCar))
    print("compassRoadNamesEnabled: " .. tostring(compassRoadNamesEnabled))
    print("inVehicle: " .. tostring(inVehicle))
end

AddEventHandler("cool-hud:watch", function()
...
AddEventHandler("cool-hud:watch:reset", function()
    showCompassFromWatch = false
    toggleCompass()
end)

RegisterNetEvent('arp-inventory:itemCheck')
AddEventHandler('arp-inventory:itemCheck', function (item, hasItem, quantity)
...
RegisterNetEvent("arp-hud:changeRange")
AddEventHandler("arp-hud:changeRange", function(proximity)
    sendAppEvent("hud", {
        voiceRange = tonumber(proximity)
    })
end)
...
AddEventHandler('onClientResourceStart', function(resourceName)
    ...
    --initHud()
    TriggerEvent("cool-ui:initHud")
end)

-- setHudValue('voiceActive', true)
-- setHudValue('voiceActiveRadio', true)

-- Citizen.CreateThread(function ()
-- 	while true do
-- 		local isTalking = NetworkIsPlayerTalking(PlayerId())
--         local pRadioActive = exports['arp-voice']:pRadioActive()
...
AddEventHandler("arp-admin:currentDevmode", function(devmode)
    setHudValue("modeDev", devmode)
end)

RegisterNetEvent("arp-admin:currentDebug")
AddEventHandler("arp-admin:currentDebug", function(debugToggle)
    setHudValue("modeDebug", debugToggle)
end)

RegisterNetEvent("carandplayerhud:godCheck", function(toggle)
    setHudValue("modeGod", toggle)
end)
...
RegisterNetEvent('arp-boosting:client:setBoostCompletions')
AddEventHandler('arp-boosting:client:setBoostCompletions', function (pAmount)
...
function getFuel()
    fuel = exports["arp-vehicles"]:CurrentFuel() or 50
end
...
function generateSpeedo()
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            setHudValue("radarShow", carHudShow)
            -- sendAppEvent("hud", {
            --     radarShow = carHudShow
            -- })
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
                harnessDurability = exports["arp-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end

            local degradation = exports["arp-vehicles"]:GetVehicleDegradation(veh)
...
-- RegisterNetEvent('arp-hud:show_weapon_out')
-- AddEventHandler('arp-hud:show_weapon_out', function()
--     sendAppEvent("hud", {
--         isArmed = true
--     })
-- end)

-- RegisterNetEvent('arp-hud:hide_weapon_out')
-- AddEventHandler('arp-hud:hide_weapon_out', function()
--     sendAppEvent("hud", {
--         isArmed = false
--     })
-- end)
...
RegisterNetEvent('arp-weapons:client:showWeaponFireRate')
AddEventHandler('arp-weapons:client:showWeaponFireRate', function (pToggle)
...
--                         --end
--                         Fuel = exports["arp-vehicles"]:CurrentFuel()
--                     else
--                         --Fuel = DecorGetInt(veh, "GetVehicleCurrentFuel")
--                         Fuel = exports["arp-vehicles"]:CurrentFuel()
--                     end
...
--                     --DecorSetInt(veh, "GetVehicleCurrentFuel", fRound(Fuel))
--                     exports["arp-vehicles"]:SetVehicleFuel(veh, fRound(Fuel))
--                     lasteupdate = 0
--                 end
...
function getVehicleInDirection(coordFrom, coordTo)
  ...
  RegisterNetEvent("arp-hud:jerry_can:refuel")
  AddEventHandler("arp-hud:jerry_can:refuel",function()
      vehicleCurrentFuel = exports["arp-vehicles"]:CurrentFuel(veh) --DecorGetInt(veh, "GetVehicleCurrentFuel")
      if exports['arp-inventory']:hasEnoughOfItem('883325847', 1) then
        local timer = (100 - vehicleCurrentFuel) * 400
        refillVehicle()
        local finished = exports["arp-taskbar"]:taskBar(timer,"Refueling")
        local veh = getVehicleClosestToMe()
    
        if finished == 100 then
            if exports['arp-inventory']:hasEnoughOfItem('883325847', 1) then
                --DecorSetInt(veh, "GetVehicleCurrentFuel", 100)
                exports["arp-vehicles"]:SetVehicleFuel(veh, 100)
                TriggerEvent('inventory:removeItem', '883325847', 1)
            else
                TriggerEvent('DoLongHudText', 'Try again!', 2)
            end
    
            local vehicleCurrentFuel = exports["arp-vehicles"]:CurrentFuel(veh) --DecorGetInt(veh, "GetVehicleCurrentFuel")
            ...
            exports["arp-vehicles"]:SetVehicleFuel(veh, endFuel)
